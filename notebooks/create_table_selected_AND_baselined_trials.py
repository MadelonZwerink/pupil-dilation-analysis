# -*- coding: utf-8 -*-
"""
Created on Tue Jul 22 18:22:55 2025
...
bl = baseline_correction(dm, col='ptrace', baseline_len=10)
dm.baseline = bl[0]
...
# Create a boolean column: False if baseline is missing, True otherwise
...
# %% Create a new column that is 1 if the trial is both selected and a baseline 
# is set, 0 otherwise

dm.incl_and_bl = dm.bl_incl * dm.trial_incl
...
for subset in subsets:
    ...
    print(total_trials)
    included = sum(subset.incl_and_bl)
    ...
    for inf_subset in ops.split(subset.participant, "inf2", "inf3", "inf4", "inf5"):
        ...
        if len(inf_subset.participant) > 0:
            ...
            print(n_trials)
            included = sum(inf_subset.incl_and_bl)
...
selected_and_bl_trials_df = pd.DataFrame(results, index=rows)
selected_and_bl_trials_df