# -*- coding: utf-8 -*-
"""
Created on Fri Jul 18 16:27:35 2025
...
# %% Perform trial exclusion with threshold to 1 so that all trials that are 
# completely missing (all nan), are set to 0, all others are set to 1
trial_excl = perform_trial_exclusion(dm, threshold=1, t_end=dm.ptrace.depth)
...
for subset in subsets:
    ...
    included_trials = sum(subset.trial_incl)
    excluded = total_trials - included_trials
    excluded_pct = round(excluded / total_trials * 100, 1)
    results['total'].append(f"{excluded} ({excluded_pct}%)")
    ...
    for inf_subset in ops.split(subset.participant, "inf2", "inf3", "inf4", "inf5"):
        ...
        if len(inf_subset.participant) > 0:
            ...
            excluded_pct = round(excluded / n_trials * 100, 1)
            results[inf_label].append(f"{excluded} ({excluded_pct}%)")
        else:
            results[inf_label].append("0 (0.0%)")  # or NaN
            
missing_trials_df = pd.DataFrame(results, index=rows)
missing_trials_df
